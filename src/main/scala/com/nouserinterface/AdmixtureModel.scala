...
object AdmixtureModel {
  ...
  def updateP(k: Int, z: Seq[DenseMatrix[Int]], X: Seq[DenseMatrix[Int]], numAlleles: Seq[Int]): Seq[DenseMatrix[Double]] = {
    numAlleles.indices.map(l => {
      ...
      z.zip(X).foreach { case (z, x) => z(::, l).toArray.zip(x(::, l).toArray).foreach {
        case (ze, xe) => d1s.update(ze, xe, d1s(ze, xe) + 1.0)
      }}
      dirichlets(d1s)
    })
  }
  ...
  def logProbQ(alpha: Double, q: DenseMatrix[Double], n: Int, k: Int): Double = {
    ...
    val (sum, runningTotal) = q.t.toArray.foldLeft((0.0, 1.0)) { case ((sum, runningTotal), v) => val nr = runningTotal * (if (v > uf) v else uf)
  ...
  def zeroState(n: Int, k: Int, loci: Int, ploidy: Int, numAlleles: Seq[Int]): State = {
    State(Seq.fill(ploidy) {
      DenseMatrix.zeros[Int](n, loci)
    }, numAlleles.map(nl => DenseMatrix.zeros[Double](k, nl)), DenseMatrix.zeros[Double](n, k), 0.0f)
  }
  ...
  case class State(z: Seq[DenseMatrix[Int]], p: Seq[DenseMatrix[Double]], q: DenseMatrix[Double], alpha: Double) {
    ...
    def /(den: Double): State = {
      State(this.z.map(a => a /:/ den.toInt), this.p.map(a => a /:/ den), this.q /:/ den, this.alpha / den)
    }
  }
}
